	...
	for (i = 0; i < plat_info->chip_nr + 1; i++) 
	{
		...
		if ( nand_page_size == 2048 )
		{
			...
			case 0 : 			
				...
				mtd_device_parse_register(s3c_mtd, NULL, NULL, partitions, 2);
				//add_mtd_partitions(s3c_mtd, partitions, 2 ); 
				break;
			case 1 : 
				printk("S3C NAND Driver is using hardware ECC-1\n");
				mtd_device_parse_register(s3c_mtd, NULL, NULL, &partitions[2], num_partitions-2);
		...
		else
		{
			...
			num_partitions = plat_info->mtd_part_nr;
			partitions     = partition_info;
			mtd_device_parse_register(s3c_mtd, NULL, NULL, partitions, num_partitions);
