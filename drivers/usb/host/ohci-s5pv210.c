/*
 * SAMSUNG s5pv210 USB HOST OHCI Controller
 *
...
static const struct hc_driver s5pv210_ohci_hc_driver = {
	...
	.product_desc		= "s5pv210 OHCI Host Controller",
...
static int __devinit s5pv210_ohci_probe(struct platform_device *pdev)
{
	...
	s5pv210_ohci = kzalloc(sizeof(struct s5pv210_ohci_hcd), GFP_KERNEL);
	if (!s5pv210_ohci)
		return -ENOMEM;
	...
	s5pv210_ohci->clk = clk_get(&pdev->dev, "usbhost");
	...
	err = clk_enable(s5pv210_ohci->clk);
	if (err)
		goto fail_clken;
...
fail:
	iounmap(hcd->regs);
fail_io:
	clk_disable(s5pv210_ohci->clk);
fail_clken:
	clk_put(s5pv210_ohci->clk);
fail_clk:
	usb_put_hcd(hcd);
fail_hcd:
	kfree(s5pv210_ohci);
...
static int __devexit s5pv210_ohci_remove(struct platform_device *pdev)
{
...
static struct platform_driver s5pv210_ohci_driver = {
	...
	.remove		= __devexit_p(s5pv210_ohci_remove),
